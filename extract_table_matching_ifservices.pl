...
if (not defined $servicefile or not defined $inputfile or not -f $servicefile or not -f $inputfile) {
	print "usage: $0 <foreignid_file> <servicefile> <input.sql>\n\n";
	exit 1;
}
...
my $dbh = DBI->connect('dbi:Pg:dbname=temp', 'postgres');
